...
def getTextImage(data, width, height, bgColor=0, color=255):
  ...
  image = Image.new("L", (width * fontWidth, height * fontHeight), color=bgColor)
  draw = ImageDraw.Draw(image)
...
def makeModel(rangeStart, rangeEnd, epochs):
  ...
  model = Sequential()
  model.add(Conv2D(32, (3, 3), activation='relu', input_shape=(fontWidth, fontHeight, 1)))
...
def getStringImage(image, model, rangeStart):
  ...
  for y in range(0, height, 1):
    for x in range(0, width, 1):
      fragment = image[y * fontHeight:(y+1) * fontHeight, x * fontWidth:(x+1) * fontWidth, np.newaxis]
...
def fromScreen(model, rangeStart):
  ...
  while True:
    ...
    frame = getStringImage(image, model, rangeStart).resize(size=(windowWidth, windowHeight))
...
def fromVideo(model, rangeStart, videoInput, videoOutput, endRange):
  ...
  out = cv2.VideoWriter(videoOutput, fourcc, fps, (int((width // fontWidth) * fontWidth), int((height // fontHeight) * fontHeight)))
...
def main(fontPath, fontSize, rangeStart, rangeEnd, epochs, inputPath, outputPath, loutputPath, videoInput, videoOutput, endRange):
  ...
  if not isfile("model.json"):
    ...
    model = loadModel()
  if inputPath:
    fromImage(model=model, rangeStart=rangeStart, inputPath=inputPath, outputPath=outputPath, loutputPath=loutputPath)
  elif videoInput:
    fromVideo(model=model, rangeStart=rangeStart, videoInput=videoInput, videoOutput=videoOutput, endRange=endRange)
  else:
...
if __name__ == "__main__":
  ...
  parser.add_argument("--font-path", required=False, type=FileType("r"), default="CascadiaMono.ttf")
  ...
  parser.add_argument("--output", required=False, type=FileType("w"), default="result.png")
  parser.add_argument("--loutput", required=False, type=FileType("w"), default="LMode.png")
  parser.add_argument("--video-input", required=False, type=FileType("r"))
  parser.add_argument("--video-output", required=False, type=FileType("w"), default="result.mp4")
  ...
  main(fontPath=args.font_path.name, fontSize=args.font_size, rangeStart=args.range_start, rangeEnd=args.range_end, \
       epochs=args.epochs, inputPath=(args.input.name if args.input else None), outputPath=args.output.name, loutputPath=args.loutput.name, \
        videoInput=(args.video_input.name if args.video_input else None), videoOutput=args.video_output.name, endRange=args.end_range);